...
const dragableTouchmove = (coordinates, _element, offset, _lastLocation) => {
...
const TinderCard = ({ flickOnSwipe = true, children, onCardLeftScreen, className, preventSwipe = [] }) => {
		...
		if (Math.abs(speed.x) > settings.swipeThreshold || Math.abs(speed.y) > settings.swipeThreshold) {
			if (flickOnSwipe) {
				if (preventSwipe.includes(getSwipeDirection(speed))) {
					...
					onCardLeftScreen(getSwipeDirection(speed));
				}
			} else {
				animateBack(element);
			}
